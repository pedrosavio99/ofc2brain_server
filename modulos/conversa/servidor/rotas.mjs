 ...
 */
import { recuperar, blocoDeNotas, blocoDoPanorama } from "./recuperacao.mjs";
...
- Voce tem opiniao. Se as notas dela se contradizem, aponte a contradicao.
...
- Referencie pelo conteudo ("aquela vez que voce escreveu que..."), nunca por id.
...
export function montarPrompt({ tracos, panorama, bloco, texto, hojeISO }) {
...
export async function rotear(req, url) {
  ...
  if (req.method === "GET" && rota === "/health") {
    return { status: "ok", modulo: "conversa" };
  }
  ...
  if (req.method === "POST" && rota === "/contexto") {
    ...
    const { turno } = await banco.inserirTurno({
      ...
      meta: { fortes: achado.fortes.length },
    });
  ...
  if (req.method === "POST" && rota === "/responder") {
    ...
    const notas = brutas.filter(Boolean).map((n) => ({ ...n, embedding: undefined, score: 1 }));

    const [alma, tracos, anteriores, panorama] = await Promise.all([
      lerAlma(),
      banco.listarTracos({}),
    ...
    if (temGemini()) {
      try {
        const g = await gerarJSONDetalhado(SISTEMA, prompt, {
          ...
          temperatura: 0.75, mensagens: historico, maxTokens: 4096,
        });
